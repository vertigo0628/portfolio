...
import { Cpu, Grid, Code, AlertTriangle, CheckCircle2 } from 'lucide-react';
import ScrollAnimation from './ScrollAnimation';

const VLauncherCaseStudy = ({ data }) => {
    if (!data) return null;

    return (
        <div className="vlauncher-casestudy mt-8">
            <ScrollAnimation delay={0.1}>
                <div className="casestudy-header mb-6">
                    <Badge variant="outline" className="mb-2 bg-pink-500/10 text-pink-400 border-pink-500/20">
                        Technical Deep Dive
                    </Badge>
                        ...
                        <CardContent className="p-0 flex flex-col h-full">
                            ...
                            <div className="mt-auto bg-[#161b22] border-t border-slate-800 p-4 font-mono text-xs overflow-x-auto">
                                <div className="flex items-center gap-2 mb-2 text-slate-500">
                                    <Code size={14} />
                                    <span>FlowerGridManager.kt</span>
                                </div>
                                <pre className="text-slate-300">
                                    <code dangerouslySetInnerHTML={{
                                        __html: data.syncLogic.codeSnippet
                                            .replace(/val/g, '<span class="text-pink-400">val</span>')
...
export default VLauncherCaseStudy;
